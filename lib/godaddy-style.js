...
function loadLintConfig(name) {
  ...
  var config = fs.readFileSync(configFilePath, { encoding: 'utf-8' });
...
module.exports = function (gulp, options) {
  ...
  gulp.task('eslint-junit', 'Lint the js files with eslint and output junit results to file', function () {
    var output = process.env.ESLINT_FILE || './build/lint/eslint-results.xml';
    ...
    return gulp.src(jscsConfig.files || options.files)
      ...
      .pipe(through2.obj(function (file, enc, callback) {
        if (file.jscs && !file.jscs.success) {
          var unfixedErrors = file.jscs.errors.getErrorList().filter(function (error) { return !error.fixed; });
          if (unfixedErrors.length) {
            fails.push(file.path);
          }
        }
        fs.writeFile(file.path, file._contents, callback);
      }, function (cb) {
        if (fails.length) {
          this.emit('error', new util.PluginError('gulp-jscs', { // eslint-disable-line no-invalid-this
