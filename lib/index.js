...
module.exports = function (gulp, options) {
  ...
  lintOptions.files = lintOptions.files || [
    options.sourceFiles,
  ...
  lintOptions.jscs.files = lintOptions.jscs.files || [
    options.sourceFiles,
    options.unitTestFiles,
    options.integrationTestFiles,
    'index.js',
  ...
  gulp.task('unit-coverage', 'Run all the integration tests with code coverage', function (cb) {
  ...
  gulp.task('mocha', 'Run all tests without code coverage', function (cb) {
    runWithDependencyProcess(runMocha, [options.unitTestFiles, options.integrationTestFiles], cb);
  });

  gulp.task('test', 'Run all the tests with code coverage', ['lint'], function (cb) {
    runWithDependencyProcess(runIstanbul, [options.unitTestFiles, options.integrationTestFiles], cb);
  });

  var description = 'Watch files for changes and invoke mocha when one changes.';
  gulp.task('watch', description, function () {
    var watchFiles = [options.sourceFiles, options.unitTestFiles, options.integrationTestFiles];
    var watcher = gulp.watch(options.watchFiles || watchFiles);
    var testFiles = [options.unitTestFiles, options.integrationTestFiles];
