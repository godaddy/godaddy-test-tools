...
module.exports = function (gulp, options) {
  ...
  options = _.defaultsDeep(options || {}, {
    ...
    functionalTestFiles: FUNCTIONAL_FILES,
    base: 'test',
    istanbul: {
  ...
  gulp.task('functional', 'Run functional tests', function (cb) {
    runWithDependencyProcess(runMocha, options.functionalTestFiles, cb);
  });
  ...
  gulp.task('functional-coverage', 'Run all the functional tests with code coverage', function (cb) {
    runWithDependencyProcess(runIstanbul, options.functionalTestFiles, cb);
  });

  gulp.task('mocha', 'Run all tests without code coverage', function (cb) {
    runWithDependencyProcess(runMocha, _.flattenDeep([options.unitTestFiles, options.integrationTestFiles, options.functionalTestFiles]), cb);
  });

  gulp.task('test', 'Run all the tests with code coverage', ['lint'], function (cb) {
    runWithDependencyProcess(runIstanbul, _.flattenDeep([options.unitTestFiles, options.integrationTestFiles, options.functionalTestFiles]), cb);
  });

  var description = 'Watch files for changes and invoke mocha when one changes.';
  gulp.task('watch', description, function () {
    var watchFiles = _.flattenDeep([options.sourceFiles, options.unitTestFiles, options.integrationTestFiles, options.functionalTestFiles]);
    var watcher = gulp.watch(options.watchFiles || watchFiles);
    var testFiles = _.flattenDeep([options.unitTestFiles, options.integrationTestFiles, options.functionalTestFiles]);
